...
const String _kCraftSellerEmail = 'myitihas@gmail.com';

class CraftShopPage extends StatelessWidget {
  const CraftShopPage({super.key, required this.hub});
  ...
  Future<void> _submitToMyItihas({
    ...
    await _supabase.from('craft_seller_submissions').insert({
      'craft_hub_id': hub.id,
      'submitted_by': userId,
      'status': 'pending',
      'payload': {
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Stored in moderation queue. If DB is unavailable, email fallback is used.',
                    ...
                    onPressed: () async {
                      ...
                      try {
                        ...
                        final body = StringBuffer()
                          ..writeln('Craft hub: ${hub.name} (${hub.id})')
                          ..writeln('Craft: ${hub.craftName}')
                          ..writeln('Seller: ${nameCtrl.text.trim()}')
                          ..writeln('Org: ${orgCtrl.text.trim()}')
                          ..writeln('Type: $sellerType')
                          ..writeln('Contact: ${contactCtrl.text.trim()}')
                          ..writeln('Email: ${emailCtrl.text.trim()}')
                          ..writeln('Address: ${addressCtrl.text.trim()}')
                          ..writeln('City: ${cityCtrl.text.trim()}')
                          ..writeln('Website: ${websiteCtrl.text.trim()}')
                          ..writeln('Notes: ${noteCtrl.text.trim()}')
                          ..writeln()
                          ..writeln('Fallback reason: $e');
                        await _openMailto(
                          context,
                          subject:
                              'Craft seller submission fallback - ${hub.name}',
                          body: body.toString(),
                        );
                      }
                    },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Craft sellers')),
      ...
      body: ListView(
        ...
        children: [
          ...
          Container(
            ...
            child: Text(
              'Curated official and trusted channels for ${hub.craftName}. '
              'Verify authenticity and current policy on official portals.',
